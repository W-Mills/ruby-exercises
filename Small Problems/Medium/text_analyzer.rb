...
class TextAnalyzer
  def process(text_file)
    ...
    yield file.read
    # file.close ## > this is automatic in later Ruby
  end
end

# File.foreach('sample_text.txt') {|x| print x }


analyzer = TextAnalyzer.new
analyzer.process('sample_text.txt') { |text| puts "#{text.split("\n\n").count} paragraphs" }
analyzer.process('sample_text.txt') { |text| puts "#{text.split("\n").count} lines"}
analyzer.process('sample_text.txt') { |text| puts "#{text.split(" ").count} words" }
